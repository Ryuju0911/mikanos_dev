...
void DrawEye(uint64_t layer_id_flags,
  ...
  SyscallWinFillRectangle(
    layer_id_flags, eye_x - kEyeSize/2, eye_y - kEyeSize/2,
...
extern "C" void main(int argc, char **argv) {
  ...
  SyscallWinFillRectangle(layer_id, 4, 24, kCanvasSize, kCanvasSize, 0xff'ffff);
  ...
  while (true) {
    ...
    if (events[0].type == AppEvent::kQuit) {
      ...
      SyscallWinFillRectangle(layer_id | LAYER_NO_REDRAW,
        4, 24, kCanvasSize, kCanvasSize, 0xff'ffff);
      DrawEye(layer_id, arg.x, arg.y, 0x00'0000);
    } else {
