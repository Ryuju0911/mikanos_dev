...
namespace {
  ...
  /** @brief Generates a 32-bit integer for CONFIG_ADDRESS. */
  ...
  /** @brief Adds the specified function to 'devices'.
   * If the function is PCI-PCI bridge, execute 'ScanBus' to the secondary bus.
   */
  ...
  /** @brief Scans each function of the specified device number.
   * If finds a valid function, execute 'ScanFunction'.
   */
  ...
  /** @brief Scans each device with the specified bus number
   * If finds a valid device, execute 'ScanDevice'.
   */
  Error ScanBus(uint8_t bus) {
  ...
  /** @brief Reads the specified MSI capability structure.
   * 
   * @param dev PCI device whose MSI capability is read
  ...
  /** @brief Writes to the specified MSI capability structure.
   * 
   * @param dev PCI device whose MSI capability is read
   * @param cap_addr Configuration space address of MSI capability register
   * @param msi_cap Written value
   */
  ...
  /** @brief Sets the specified MSI register. */
  ...
  /** @brief Sets the specified MSI-X register. */
...
namespace pci {
  ...
  Error ConfigureMSI(const Device &dev, uint32_t msg_addr, uint32_t msg_data,
    ...
    if (msi_cap_addr) {
      return ConfigureMSIRegister(dev, msi_cap_addr, msg_addr,
                                  msg_data, num_vector_exponent);
    } else if (msix_cap_addr) {
      return ConfigureMSIXRegister(dev, msix_cap_addr, msg_addr,
                                   msg_data, num_vector_exponent);
    }
