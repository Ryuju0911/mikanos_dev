...
WithError<uint64_t> CopyLoadSegments(Elf64_Ehdr *ehdr) {
  ...
  for (int i = 0; i < ehdr->e_phnum; ++i) {
    ...
    // set up pagemaps as read only (writable = false)
...
// Properly places the LOAD segment in the ELF file.
...
WithError<int> Terminal::ExecuteFile(fat::DirectoryEntry &file_entry,
  ...
  auto argbuf = reinterpret_cast<char*>(
    args_frame_addr.value + sizeof(char**) * argv_len);
  int argbuf_len = 4096 - sizeof(char**) * argv_len;
  auto argc = MakeArgVector(
    command, first_arg, argv, argv_len, argbuf, argbuf_len);
  if (argc.error) {
    return { 0, argc.error };
  }
