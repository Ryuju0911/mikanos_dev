...
IntHandlerLAPICTimer:  ; void IntHandlerLAPICTimer();
    ...
    ; Builds a TaskContext type structure on the stack
    sub rsp, 512
    fxsave [rsp]
    push r15
    push r14
...
SyscallEntry:  ; void SyscallEntry(void);
    ...
    ; Prepare for executing system call on the stack for OS
    ...
    ; rbx and r12-15 are callee-saved, so they are not saved by caller.
    ; rax is for return value, so it is not saved by caller.
...
ExitApp:
    ...
    ret  ; Jump tp the next line of the caller of CallApp
